...
public static int jacobian_partial(matrix A, matrix V, vector e, int deg=0){
	...
	int p, q, change, sweep=0, n=A.size1;
	V.set_unity(); // Sets V matrix to identity matrix
	vector e_temp = new vector(n);
	for(int i = 0; i<e.size; i++)e_temp[i]=A[i,i];
	// So far, we're just looking at the first row
	p=0;
	for(q=p+1; p<n; p++){
		double[] Aold = {e_temp[p], e_temp[q], A[p,q]};
		double phi = Math.Atan2(2*Aold[2], Aold[1]-Aold[0])/2;
		double c = Math.Cos(phi);
		double s = Math.Sin(phi);
		double[] Anew = new double[3];
