...
// Making the sinh, cosh and sqrt(z), where z is complex

public static double sinh (double x){return (Math.Exp (x) - Math.Exp(-x))/2;}
...
public static complex sqrt (complex z){
	var R = sqrt( (sqrt(pow(z.Re,2)+pow(z.Im,2))+z.Re)/2);
	var I = sqrt( (sqrt(pow(z.Re,2)+pow(z.Im,2))-z.Re)/2);
	if (z.Im >= 0){
